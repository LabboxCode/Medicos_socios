...
def get_diario_week(week_start: date, week_end: date) -> list[dict]:
    ...
    q = """
    query($boardId: ID!, $from: String!, $to: String!) {
      boards(ids: [$boardId]) {
        items_page(limit: 500, query_params: {
          rules: [
            {column_id: "date", compare_value: [$from, $to], operator: between}
          ]
        }) {
          cursor
          items {
    ...
    cursor = None
    while True:
        pq = q if not cursor else q.replace(
            "items_page(limit: 500",
            f'items_page(limit: 500, cursor: "{cursor}"'
        )
        data = gql(pq, {
            "boardId": str(BOARD_DIARIO),
            "from": fmt_iso(week_start),
            "to":   fmt_iso(week_end),
        })
...
def get_new_recetarios(since: date) -> list[dict]:
    ...
    q = """
    query($boardId: ID!, $since: String!) {
      boards(ids: [$boardId]) {
        items_page(limit: 500, query_params: {
          rules: [
            {column_id: "fecha_Mjj4f1Wb", compare_value: $since, operator: greater_than_or_equals}
    ...
    """
    data = gql(q, {"boardId": str(BOARD_PLANES), "since": fmt_iso(since)})
...
def get_new_socios(since: date) -> list[dict]:
    ...
    """
    data = gql(q, {"boardId": str(BOARD_PLANES)})
